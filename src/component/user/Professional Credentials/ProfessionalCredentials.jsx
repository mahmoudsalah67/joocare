...
function ProfessionalCredentials() {
  ...
  const fetchQualifications = async () => {
    try {
      const res = await axios.get('https://joocare.nami-tec.com/api/user/qualifications', { headers });
  ...
  const openAddModal = () => {
    setEditingItem(null);
    setFormData({ degree: '', university: '', country_id: '', start_date: '', end_date: '', image: null });
  ...
  const openEditModal = (item) => {
    ...
    setFormData({
      ...
      country_id: item.country_id || item.country?.id || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingItem) {
        //  Edit - POST /qualifications/{id} مع _method=PUT
        payload._method = "PUT";
        await axios.post(
          `https://joocare.nami-tec.com/api/user/qualifications/${editingItem.id}`,
          payload,
          { headers: { ...headers, "Content-Type": "application/json" } }
        );
        alert("تم تحديث المؤهل بنجاح!");
      } else {
        //  Add - POST /qualifications
        await axios.post(
          'https://joocare.nami-tec.com/api/user/qualifications',
          payload,
          { headers: { ...headers, "Content-Type": "application/json" } }
        );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `https://joocare.nami-tec.com/api/user/qualifications/${id}`,
        { headers }
      );
      fetchQualifications();
    } catch (err) {
        ...
        <main className="lg:col-span-9 flex flex-col gap-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <button onClick={openAddModal} className="flex items-center gap-2 px-5 py-3 bg-[#1A252C] text-white text-xs font-bold rounded-full hover:bg-black transition">
              <FiPlusCircle size={15} /> Add Qualifications
            </button>
          </div>
            ...
            {qualifications.map((item) => (
              <div key={item.id} className="bg-white border border-gray-100 p-4 rounded-2xl flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-[#0D0D0D]">{item.degree}</h3>
          ...
          <div className="bg-white rounded-[24px] w-full max-w-[560px] p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <h2 className="text-[22px] font-bold text-[#0D0D0D]">
                {editingItem ? "Edit Qualification" : "Add Qualifications"}
              </h2>
