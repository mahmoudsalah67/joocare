...
function Dashboard() {
    ...
    const fetchProfile = async () => {
      try {
        ...
        const res = await axios.get(
          "https://joocare.nami-tec.com/api/company/auth/profile",
          { headers }
        );
  ...
  if (loading)
    return (
      <div className="flex items-center justify-center min-h-screen text-[#00694B] font-bold">
        Loading...
      </div>
    );
  if (error)
            ...
            <div className="flex flex-col lg:flex-row gap-6 px-4 sm:px-8 lg:px-10 w-full">
              ...
              <aside className="w-full lg:flex-none lg:basis-1/4 h-fit flex flex-col gap-4 bg-white pt-[60px] sm:pt-[90px] lg:pt-[120px] mt-0 lg:mt-[-80px] pb-8 px-[12px] rounded-b-[32px] shadow-sm">
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-[14px] text-[#4D4D4D]">
                        {profileData?.city?.title || "Egypt"}
                      </p>
                    </div>
                  </div>
                  <p className="text-[12px] text-[#8F8F8F] leading-relaxed mb-4 line-clamp-3">
                    {profileData?.bio ||
                      "No description provided yet. Click edit to add an about section."}
                  </p>
                  <div
                    className={`text-center py-2 rounded-full text-[13px] font-semibold border ${
                ...
                <div className="flex flex-col gap-1 px-2 mt-2">
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `flex items-center gap-3 p-3 rounded-full font-medium transition-all ${
                        isActive
                          ? "bg-[#E6F3EF] text-[#00694B] font-bold"
                          : "text-[#8F8F8F]"
                      }`
                    }
                  >
                  ...
                  >
                    <img src={Icon2} className="text-xl" alt="" /> Job
                    Management
                  </NavLink>
                  <NavLink
                    to="/Accountsettings"
              ...
              <main className="w-full lg:flex-1 py-10 lg:py-[70px]">
                <div className="profile-details px-4 sm:px-8 lg:px-[40px] py-6 sm:py-[32px] bg-[#F7FAF7] min-h-screen">
                  <h2 className="text-[18px] sm:text-[20px] font-bold text-[#0D0D0D] mb-6">
                    Active Jobs
                  </h2>

                  {/* Stats Cards */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8">
                    <div className="bg-white p-5 sm:p-6 rounded-[24px] shadow-sm border border-gray-50 relative overflow-hidden">
                      <p className="text-[14px] font-bold text-[#0D0D0D] mb-2">
                        Active job postings
                      </p>
                      <h3 className="text-[28px] sm:text-[32px] font-bold text-[#00694B]">
                        {dashboardData?.active_jobs || 0}
                      </h3>
                      <div className="absolute right-6 top-1/2 -translate-y-1/2 opacity-20">
                        <img src={jobIcon} className="w-12 h-12" alt="" />
                      </div>
                    </div>

                    <div className="bg-white p-5 sm:p-6 rounded-[24px] shadow-sm border border-gray-50">
                      <p className="text-[14px] font-bold text-[#0D0D0D] mb-2">
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-[12px] font-bold text-[#0D0D0D]">
                          +{dashboardData?.applications_this_week || 0}{" "}
                          <span className="font-normal text-[#8F8F8F]">
                            this week
                          </span>
                        </span>
                        <img src={people} alt="" />
                    ...
                    <div className="bg-white p-5 sm:p-6 rounded-[24px] shadow-sm border border-gray-50">
                      ...
                      <div className="flex items-center justify-between">
                        <h3 className="text-[28px] sm:text-[32px] font-bold text-[#00694B]">
                          +{dashboardData?.applications_latest_activity || 0}{" "}
                  ...
                  <div className="flex flex-col lg:flex-row gap-6">
                    <div className="w-full lg:basis-1/3 space-y-6">
                      <div className="bg-white p-5 sm:p-6 rounded-[24px] shadow-sm border border-gray-50">
                        <p className="text-[14px] font-bold text-[#0D0D0D] mb-2">
                          Talent intake
                        </p>
                        <div className="flex items-center justify-between">
                          <h3 className="text-[28px] sm:text-[32px] font-bold text-[#00694B]">
                            {dashboardData?.cvs_downloaded || 0}
                          </h3>
                          <img src={Correct} alt="" />
                        </div>
                      </div>
                      ...
                      <div className="bg-white p-5 sm:p-6 rounded-[24px] shadow-sm border border-gray-50">
                        <h4 className="text-[16px] font-bold text-[#0D0D0D] mb-6">
                          Category Breakdown
                        </h4>
                        <div className="flex items-center justify-between gap-4">
                          <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full border-[16px] sm:border-[20px] border-l-[#00694B] border-b-[#FFB800] border-r-[#4A90E2] border-t-[#7ED321]"></div>
                          <div className="text-[12px] space-y-2">
                            <div className="flex items-center gap-2">
                              <span className="w-3 h-3 bg-[#4A90E2] rounded-sm"></span>{" "}
                              nursing
                            </div>
                            <div className="flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-2">
                              <span className="w-3 h-3 bg-[#FFB800] rounded-sm"></span>{" "}
                              Allied Health
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="w-full lg:flex-1 bg-white p-5 sm:p-6 rounded-[24px] shadow-sm border border-gray-50">
                      <h4 className="text-[18px] font-bold text-[#0D0D0D] mb-4">
                        Active Jobs
                      </h4>
                                  ...
                                  <td className="p-4 flex gap-2">
                                    ...
                                    <button
                                      onClick={() =>
                                        handleDownloadCV(
                                          job.id,
                                          job.cv_url || job.job_title?.cv_url
                                        )
                                      }
                  ...
                  {/* Pagination */}
                  <div className="flex justify-center gap-[20px] items-center mt-6 text-[12px] text-[#4D4D4D]">
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() =>
                          setCurrentPage((prev) => Math.max(prev - 1, 1))
                        }
                        className="w-8 h-8 flex items-center justify-center rounded-full border border-gray-200 hover:bg-gray-100 transition cursor-pointer"
                      >
                        ‹
                      </button>
                      {[1, 2, 3, 4, 5].map((n) => (
