...
const EmailVerificationModal = ({ email, onClose }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
       const response = await axios.post('https://joocare.nami-tec.com/api/user/auth/email/confirm', {
        email: email,  
        ...
        <div className="flex justify-center gap-3 mb-8">
          {otp.map((data, index) => (
            <input
              ...
              className="w-[65px] sm:w-[75px] h-[85px] sm:h-[95px] bg-[#E1E3E1] border border-[#C4C7C5] rounded-[20px] text-center text-3xl sm:text-4xl font-bold text-[#1A1C1E] focus:border-[#00694B] focus:ring-2 focus:ring-[#00694B]/20 outline-none transition-all"
            />
          ))}
        </div>
