      ...
      >
        <div className="section-home mt-50 pb-20">
          <div className="container mx-auto px-4 lg:px-30">
            ...
            {/* Search Bar Section */}
<div className="search-bar px-4 md:px-10 lg:p-5 mt-10 md:mt-20 relative">
  <div className="container mx-auto">
    <div className="bg-[#F1F3F5] rounded-[30px] lg:rounded-[100px] p-2 shadow-inner">
      {/* تحويل الـ Bar لـ Grid مرن */}
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] items-center gap-2">
        
        {/* Input Field */}
        <div className="flex items-center gap-3 bg-white px-5 py-3 rounded-full w-full" ref={suggestionsRef}>
          <FiSearch className="text-gray-400 cursor-pointer shrink-0" onClick={handleSearch} />
          <input
            type="text"
            value={term}
        ...
        {/* Dropdowns & Button Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-flow-col lg:auto-cols-max items-center gap-2 w-full lg:w-auto">
          {/* By Country */}
          <div className={`relative ${dropdownStyle} w-full`}>
            <select
            ...
            <MdKeyboardArrowDown className="text-gray-500 shrink-0" size={18} />
          </div>
          ...
          {/* By Category */}
          <div className={`relative ${dropdownStyle} w-full sm:col-span-2 lg:col-span-1`}>
            <select
              className="absolute inset-0 opacity-0 cursor-pointer z-10 w-full"
              onChange={(e) => {
          ...
          <Link
            ...
            className="bg-[#00694B] text-white px-8 py-3 rounded-full font-semibold text-sm hover:bg-black transition-colors text-center w-full lg:w-auto sm:col-span-2 lg:col-span-1"
          >
            Find Jobs
          </Link>
        </div>
...
<div className="px-4 md:px-[24px]">
  <div className="container mx-auto py-5 lg:py-8">
    <div className="grid grid-cols-1 lg:grid-cols-[auto_1fr_auto] items-start gap-4 lg:gap-6">
      
      {/* Title */}
      <h2 className="text-[16px] md:text-[18px] font-bold text-[#1A1A1A] lg:pt-2 whitespace-nowrap">
        Popular Searches
      </h2>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap gap-2 md:gap-3">
          {searchData?.data?.slice(0, 4).map((job, index) => (
            <Link to={`/jobs?search=${job.word}`} key={index} className="shrink-0">
              <div className="flex items-center gap-2 px-3 py-2 md:px-[12px] md:py-[8px] border border-[#E5E7EB] rounded-full hover:bg-gray-50 cursor-pointer transition">
                <FiSearch className="text-gray-400 text-sm" />
                <span className="text-[12px] md:text-[14px] font-medium text-[#00694B] whitespace-nowrap">{job?.word}</span>
              </div>
            </Link>
          ))}
        </div>
        ...
        {/* Expandable Content */}
        <div className={`grid transition-all duration-500 ease-in-out ${showAll ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
          <div className="overflow-hidden">
            <div className="flex flex-wrap gap-2 md:gap-3 pt-2">
              {searchData?.data?.slice(4).map((job, index) => (
                <Link to={`/jobs?search=${encodeURIComponent(job.word)}`} key={index} className="shrink-0">
                  <div className="flex items-center gap-2 px-3 py-2 md:px-[12px] md:py-[8px] border border-[#E5E7EB] rounded-full hover:bg-gray-50 cursor-pointer transition">
                    <FiSearch className="text-gray-400 text-sm" />
                    <span className="text-[12px] md:text-[14px] font-medium text-[#00694B] whitespace-nowrap">{job?.word}</span>
                  </div>
      ...
      <div className="flex items-center justify-start lg:justify-end">
        <button
          ...
          className="flex items-center gap-2 p-2 rounded-full transition duration-500 hover:bg-[#00694B] group"
        >
          <span className="text-[14px] font-bold text-[#00694B] group-hover:text-white transition-all">
            {showAll ? "Show Less" : "Show More"}
          </span>
          <BsArrowUpRightCircle className={`size-[18px] text-[#00694B] group-hover:text-white transition-transform duration-500 ${showAll ? "rotate-45" : "rotate-0"}`} />
        </button>
      </div>
        ...
        <div className="section-about pt-32 pb-20">
          <div className="container mx-auto px-4 lg:px-30">
            <div className="content">
              ...
              {/*  flex-col on mobile */}
              <div className="container-small flex flex-col lg:flex-row justify-center gap-12">
                <div className="item1 flex flex-col items-center justify-center gap-[16px] mt-16">
                  ...
                  <div className="title text-center">
                    <h2 className="p-[12px] text-[#0D0D0D] text-2xl font-bold">Precision Profiling</h2>
                    <p className="text-[16px] text-[#0D0D0DA6]">{homeData?.data?.how_it_works_section?.steps[0]?.description}</p>
                  </div>
                </div>
                ...
                <div className="item2 flex flex-col items-center justify-center text-center gap-[16px] mt-16">
                  ...
                  <div className="title">
                    <h2 className="p-[12px] text-[#0D0D0D] text-2xl font-bold">Intelligent Matching</h2>
                    <p className="text-[16px] text-[#0D0D0DA6]">{homeData?.data?.how_it_works_section?.steps[1]?.description}</p>
                  </div>
                </div>
                ...
                <div className="item3 flex flex-col items-center text-center justify-center gap-[16px] mt-16">
                  ...
                  <div className="title">
                    <h2 className="p-[12px] text-[#0D0D0D] text-2xl font-bold">Verified Placement</h2>
                    <p className="text-[16px] text-[#0D0D0DA6] leading-relaxed">{homeData?.data?.how_it_works_section?.steps[2]?.description}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="frame">
            <div className="content pt-32 pb-20">
              ...
              <div className="whycontainer relative flex flex-col md:flex-row w-full overflow-hidden">
                    ...
                    <div className="space-y-10">
                      {[0, 1, 2].map((i) => (
                        <div key={i} className="flex">
                          <div className="w-[48px] h-[48px] p-[10px] bg-[#00694B] rounded-full flex items-center justify-center text-white text-[20px] shadow-2xl shrink-0">
                ...
                <div className="absolute top-80 left-[50%] -translate-x-1/2 -translate-y-1/2 hidden md:block">
                  <img src={robothand} alt="robothand" className="robothand h-[40px] translate-x-10" />
                </div>
                <div className="absolute z-10 top-80 left-[50%] -translate-x-1/2 -translate-y-1/2 hidden md:block">
                  <img src={cont2} alt="con2" className="cont2 h-[100px] translate-x-5" />
                </div>
                <div className="top absolute top-1/2 left-[50%] -translate-x-15 -translate-y-1/2 hidden md:block">
                  <img src={top} alt="top" className="translate-x-8 w-auto max-w-none" />
                </div>
                <div className="frame absolute top-80 left-[49.5%] -translate-x-30 -translate-y-1/2 hidden md:block">
        ...
        <div className="TopEmployers pb-20">
          <div className="container mx-auto px-4 lg:px-30">
            <div className="content">
              <div className="title-1 flex items-center justify-center mx-auto gap-2 mb-6 border-[1px] border-[#2E90A61A] rounded-[12px] py-[8px] px-[16px] w-fit bg-[#12121205]">
                <img src={star2} alt="" />
                <p className="text-[#1C2628] text-[16px] font-[400]">Top Employers</p>
              </div>
            ...
            <div className="content flex flex-col lg:flex-row items-center justify-between gap-12">
              ...
              <div className="cards grid grid-cols-2 gap-[16px] w-full">
                {[
                  ...
                  { num: "+100", label: "Active Job opportunities" },
                ...
                <div className="right">
                  <div className="flex items-center gap-2 ml-4 cursor-pointer shrink-0">
                    <div className="flex items-center gap-1 p-2 rounded-full transition duration-500 hover:bg-[#00694B] hover:text-white">
                      <NavLink to={"jobs"} className="font-bold text-gray-[#0D0D0DA6] text-[16px] font-[400]">Explore More</NavLink>
                  ...
                  <div className="contanier grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[16px] w-full">
                    {homeData?.data?.recent_jobs?.jobs?.map((job) => (
                      <Link to={`job/${job.id}`} key={job.id}>
                        <div className="flex bg-[#e0dddd1f] border-[2px] border-transparent mb-[16px] hover:shadow-2xl duration-500 hover:border-green-700 hover:cursor-pointer p-[16px] rounded-[16px]">
                          ...
                          <div className="details ml-[8px] overflow-hidden">
                            ...
                            <div className="flex items-center gap-[4px] mt-[8px] flex-wrap">
                              <span className="bg-[#0D0D0D0D] text-[gray] py-[4px] px-[8px] rounded-[8px] text-[12px]">{job.employment_type?.title}</span>
                              <p className="bg-[#0D0D0D0D] text-[gray] py-[4px] px-[6px] rounded-[8px] text-[12px]">{job.current_status?.created_at}</p>
                            </div>
                          </div>
                        </div>
                      </Link>
              ...
              {homeData?.data?.rates?.rates?.map((testimonial) => (
                <SwiperSlide key={testimonial.id}>
                  <div className="bg-white p-6 rounded-tl-[30px] rounded-br-[30px] shadow-sm border border-gray-100 mb-[132px]">
                    ...
                    <p className="text-[14px] text-gray-400 mb-4">{testimonial.date}</p>
                    <p className="text-[#4D4D4D] text-[16px] leading-relaxed">{testimonial.comment}</p>
                  </div>
                </SwiperSlide>
              ))}
            </Swiper>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-[16px] w-full">
                <div className="flex flex-col gap-[16px]">
                  {homeData?.data?.faqs?.faqs?.filter((_, index) => index % 2 === 0).map((item) => (
                    <FaqItem key={item.id} item={item} openIndex={openIndex} setOpenIndex={setOpenIndex} />
                  ))}
                </div>
                <div className="flex flex-col gap-[16px]">
                  {homeData?.data?.faqs?.faqs?.filter((_, index) => index % 2 !== 0).map((item) => (
